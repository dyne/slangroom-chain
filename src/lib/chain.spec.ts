...
test('keyTransform should work [json]', async (t) => {
  const steps = {
    verbose: true,
    steps: [
      {
...
test('callbacks should work [json]', async (t) => {
  let before = false;
  let after = false;
  let afterResult = '';
...
test('read from file [yaml]', async (t) => {
  const steps = `
    steps:
      - id: 'from file'
...
test('read from file [json]', async (t) => {
...
test('read input data [yaml]', async (t) => {
  const steps = `
    steps:
      - id: 'from file'
        zencodeFromFile: 'test_contracts/hello.zen'
...
test('read input data [json]', async (t) => {
  const steps = {
    steps: [
      {
        id: 'from input data',
