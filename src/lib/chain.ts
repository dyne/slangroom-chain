...
import { JsonChain } from './jsonChain';
import type {
  JsonSteps,
  Results,
  Step,
} from './types';
import { YamlChain } from './yamlChain';
...
export const execute = async (
  ...
  for (const step of parsedSteps.steps.steps) {
    ...
    data = await parsedSteps.manageTransform(step.dataTransform, data, 'data', verboseFn)
    keys = await parsedSteps.manageTransform(step.keysTransform, keys, 'keys', verboseFn);
    ...
    await parsedSteps.manageAfter(step.onAfter,
      stringResult,
      zencode,
      data,
      keys,
